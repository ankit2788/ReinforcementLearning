...
class Portfolio_MultiStage(Environment):
    metadata = {"render.modes": ["human"]}
    ...
    def __init__(self, assets, initialWeight, \
        ...
        # initialize state and action space
        self.action_space       = ActionManager.DiscreteActionSpace(actions)
        self.observation_space  = StateManager.StateSpace_Multi(initialState= None)
    ...
    def getcurrentState(self):
        ...
        # get price tensor
        currentPrice    = self.currentInfo.Price.Close
        currentPrice    = np.broadcast_to(currentPrice.reshape(-1, 1), np.array(self.History.Price).shape)

        state_price     = np.divide(np.array(self.History.Price), currentPrice)
        state_price     = state_price.transpose(1,0,2)          # shift axis of nbassets and history

        state_holding   = self.currentInfo.Holdings * self.currentInfo.Price.Close
        state_holding   = state_holding/ self.currentInfo.Portfolio

        state_cash      = self.currentInfo.Cash/self.currentInfo.Portfolio
        state_actions   = self.currentInfo.PossibleActions

        currentStatetuple    = (state_price, state_holding, state_cash, state_actions)
