...
from RLLibrary.FinUseCases.OrderExecution.ModelManager.A3C import NetworkManager, MemoryManager
...
class MasterAgent():
    ...
    def initNetwork(self, **networkArgs):
        ...
        if self.__ActorCriticModel is None:
            globalModel = NetworkManager.ActorCritic_FF(state_size=self.state_size, action_size=self.action_size, \
...
class WorkerAgent(threading.Thread):
    ...
    def __init__(self, envName,  \
        ...
        self.envName = envName
        # self.envName = "OrderExecution-v0"
        ...
        if ActorCriticModel is None:
            localModel = NetworkManager.ActorCritic_FF(state_size=self.state_size, action_size=self.action_size, \
                                                    actorHiddenUnits = actorHiddenUnits, criticHiddenUnits = criticHiddenUnits) 
    ...
    def run(self):
        ...
        while self.worker_episode_index.value <= self.MAX_EPISODES:
            ...
            with self.worker_episode_index.get_lock():
                ...
                Logger.info(f'{self.Name} Ep#: {self.worker_episode_index.value} Date: {self.env.envDate} Reward: {np.round(episodic_reward,1)} , Steps: {episodic_steps} INV: {inventory} Time: {self.env.currentInfo.TimeLeft}')
    ...
    def updateEpisodicInfo(self, episodeCount, episodeReward, steps, **grads):
        ...
        self.Tensorboard.update_stats(AvgReward = avg_reward, MaxReward = max_reward, \
                                        steps = steps , \
                                        #**grads \
                                        )
...
#         if self.__ActorCriticModel is None:
#             globalModel = Networks.ActorCritic_CNN(nbHistory = self.globalEnvironment.nbHistory, action_size=self.action_size) 
...
#         # construct the global Model with some random initial state
#         _randomInitialState = self.globalEnvironment.observation_space.currentState
#         self.globalModel(_randomInitialState)
#         print(self.globalModel.summary())
...
#         allYears = np.arange(2006,2019)
#         startTime = time.perf_counter()
#         # create individual processes
#         processes = []
#         for _thread in range(self.cores):

#             envargs = self.envargs.copy()
#             year = np.random.choice(allYears)
#             envargs = self.envargs
#             envargs["startDate"] = f'{year}-01-01'
#             envargs["endDate"] = f'{year}-12-31'

#             # create each agent object
#             worker = WorkerAgent_Multi(envName = self.envName,  \
#                     global_model = self.globalModel, sharedOptimizer = self.optimizer, \
...
#         Logger.info(f'Time taken with {self.cores} cores: {np.round(endTime - startTime), 2} secs')
#         print(f'Time taken with {self.cores} cores: {np.round(endTime - startTime), 2} secs')
...
# class WorkerAgent_Multi(threading.Thread):
#     # inheriting multiprocesing properties from Threading class
#     best_score = -np.inf
...
#         # create the local model
#         if ActorCriticModel is None:
#             localModel = Networks.ActorCritic_CNN(nbHistory=self.env.nbHistory,  action_size=self.action_size) 
...
#                 # sample the action based on prob distribution from local model
#                 probs, _ = self.localModel(current_state)
...
#             reward_sum = 0
#         else:
#             _, values = self.localModel(next_state)
...
#         # forward run the netwrok to get probabilities and value functions for all states in memory         
#         probs, values = self.localModel(memory.states)
