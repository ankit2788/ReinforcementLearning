...
class QLearningAgent(QAgentBase):
    ...
    def __init__(self, env, configFile,  **kwargs):
        ...
        self.Qmodel             = TabularModel(self.config, self.env.observation_space.n, self.env.action_space.n)
        self.Qmodel.init()
...
class FittedQAgent(QAgentBase):
    ...
    def __init__(self, env, configFile,  model = None,  **kwargs):
        ...
        self.Qmodel     = model
        if model is None:
            self.Qmodel = BaselineModel(self.config, dim_input = self.env.observation_space.n, \
...
class DQN(QAgentBase):
    ...
    def __init__(self, env, configFile,  model = None, **kwargs):
        ...
        self.Qmodel      = DeepNeuralModel(self.config, dim_input=self.env.observation_space.n, \
                                dim_output = self.env.action_space.n, \
                                time = self.__time, Name = "QModel", **kwargs)
            

        self.Targetmodel = DeepNeuralModel(self.config, dim_input=self.env.observation_space.n, \
                                dim_output = self.env.action_space.n, \
